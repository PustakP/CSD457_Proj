/*
 * button_demo_fixed.ino - FIXED VERSION with better serial handling
 * fixes:
 * - ensures complete hex transmission
 * - adds message length verification
 * - better float formatting
 */

// ---- config ----
#define BAUD_RATE 9600
#define MAX_MESSAGE_LEN 256  // increased from 128 to be safe
...
void setup() {
  ...
  // send init msg
  Serial.println("# INIT: " DEVICE_ID " ready (FIXED VERSION)");
...
void sendSensorData() {
  ...
  // convert floats to strings first to ensure proper formatting
  char tempStr[10];
  char humStr[10];
  dtostrf(simTemp, 4, 1, tempStr);  // width=4, precision=1
  dtostrf(simHumidity, 4, 1, humStr);
  
  // build json msg with string-formatted floats
  int msgLen = snprintf(msgBuffer, MAX_MESSAGE_LEN,
    "{\"id\":\"%s\",\"seq\":%lu,\"t\":%s,\"h\":%s,\"l\":%d,\"ts\":%lu}",
    DEVICE_ID, messageCounter, tempStr, humStr, simLight, millis()
  );
  ...
  // debug: show plaintext length
  Serial.print("# DEBUG: plaintext_len=");
  Serial.print(msgLen);
  Serial.print(", msg=");
  ...
  // send as hex with flush to ensure complete transmission
  Serial.print("ENC:");
  for (int i = 0; i < msgLen; i++) {
